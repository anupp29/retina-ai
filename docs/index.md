---
layout: default
title: "Retina AI: Multimodal Student Dropout Risk Prediction"
---
...
<div class="rationale-block">
    <div class="rationale-title">Design Choice & Rationale: Trajectory Beats Snapshot</div>
    <div class="rationale-content">
        Rather than taking a static snapshot of student data (a tabular classification approach), this architecture is built on the core finding that <strong>students are better described by how they are changing than by where they currently stand</strong>. Features evaluating trends consistently outperformed static indicators across all validation experiments.
    </div>
</div>
...
<div class="rationale-block">
    <div class="rationale-title">Design Choice & Rationale: Late Fusion Strategy</div>
    <div class="rationale-content">
        A multi-stream architecture using <strong>late probability-level fusion</strong> was selected over end-to-end joint optimization. Experimentation revealed that simply combining modalities early in the pipeline did not guarantee better performance; isolating independent signal streams (GBM for structured trends, BiLSTMs for temporal patterns) and blending their final probability outputs proved more robust.
    </div>
</div>
...
<div class="rationale-block">
    <div class="rationale-title">Design Choice & Rationale: Attendance Sequences Matter</div>
    <div class="rationale-content">
        While basic attendance statistics were engineered, raw temporal sequences were explicitly retained and fed into a BiLSTM. This decision was driven by the observation that <strong>the shape of the attendance curve carries meaningful predictive information</strong>—learning behavioral patterns that aggregate statistics like mean or standard deviation completely fail to capture.
    </div>
</div>
...
<div class="rationale-block">
    <div class="rationale-title">Design Choice & Rationale: Domain Knowledge Matters</div>
    <div class="rationale-content">
        Instead of relying on complex, automated NLP pipelines (like LLM embeddings), domain knowledge was directly incorporated through a manually engineered Note Severity Score. This choice was validated when simple domain expertise—quantifying the severity of interventions—outperformed generic text representations and became one of the strongest predictive signals in the final model.
    </div>
</div>
...
<div class="rationale-block">
    <div class="rationale-title">Design Choice & Rationale: Prioritizing High-Risk Identification</div>
    <div class="rationale-content">
        Multiple aggressive interventions (class-weighted loss, balanced trees, and post-processing inference optimization) were layered to combat class imbalance. The operational justification is critical: <strong>the most costly mistake in an intervention system is classifying a High-Risk student as Low-Risk</strong>. These choices successfully eliminated this specific failure mode (0 false negatives for severe cases).
    </div>
</div>
